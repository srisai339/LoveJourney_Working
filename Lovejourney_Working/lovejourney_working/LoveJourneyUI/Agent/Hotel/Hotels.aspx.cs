    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            if (!IsPostBack)
            {
                ...
                else { Response.Redirect("~/Agent/Hotel/HotelSearch.aspx", false); }
            }
        }
        catch (Exception ex)
        {
            ...
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                if (ViewState["MarkUp"] == null)
                {
                    ...
                    if (objDataSet != null)
                    {
                        ...
                        {
                            DataRow[] markUpAmountRow = objDataSet.Tables[0].Select("Type = 'Hotels'");
                            if (markUpAmountRow.Length > 0)
                            {
                                markUpAmount = markUpAmountRow[0]["MarkUpAmount"].ToString();
                                ViewState["MarkUp"] = markUpAmount;
                            }
                        }
                    }
                ...
                lbtnFare.ToolTip = " RoomRate: " + (RoomTtl + Convert.ToDouble(agentMakrUpFare)) + " \n Extra Guest Charge: "
        ...
        try
        {
            if (Session["SearchParams"] != null)
            {
                if (e.CommandName == "BookRoom")
                {
                    ...
                    totalINR = lbtnFare.CommandName.ToString();
                    string fareDetails = lbtnFare.CommandArgument.ToString();
