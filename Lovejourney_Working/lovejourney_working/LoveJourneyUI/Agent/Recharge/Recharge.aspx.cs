...
public partial class Agent_Recharge_Recharge : System.Web.UI.Page
{
    ...
    protected void loadDropdown()
    {
        ...
        //   ("http://api.buysmart.co.in/hydrasales/services/thirdpartyapi.asmx/ListProducts?GUID=" + "FCC4FE618FD28CE4" + "&basketID=" + "10");
        //request.Method = "GET";
        ...
        //    ddlProvider.DataSource = dt.DefaultView;
        //    ddlProvider.DataTextField = "ProductName";
        //    ddlProvider.DataValueField = "ProductID";
        ...
        //    Session["ProductsList"] = ds.Tables["Prod"];
    ...
    protected void btnMobileRecharge_Click(object sender, EventArgs e)
    {
        try
        {
            if (txtMobile.Text != "" && ddlProvider.SelectedItem.Text != "Please Select" && ddlMobilerechargeamount.Text != "")
            {
            ...
            if (_objDataSet.Tables[0].Rows.Count == 0)
            {
                ...
                else if (Session["Role"].ToString() == "Agent")
                {
                    ...
                    else
                    {
                        ...
                        lbldacdLowbalance.Text = "Recharge failed.Please contact administrator.";
    ...
    //        HttpWebRequest request = (HttpWebRequest)WebRequest.Create
    //            ("http://api.buysmart.co.in/hydrasales/services/thirdpartyapi.asmx/PurchaseProduct?GUID=" + "FCC4FE618FD28CE4" + "&productID=" + Session["RProviderName"] + "&typeOfTransaction=" + Session["TypeOfTranscation"] + "&amount=" + Session["RRechargeAmount"] + "&subScriberID=" + Session["RMobileNumber"] + "&UID=" + Session["Order_Id"]);       
    ...
    //            Session["Afterrecharge"] = ds3.Tables[0];
    //            Session["Afterrecharge1"] = ds3.Tables[1];

    //            //ddlMobilerechargeamount.DataSource = ds3.Tables["errorDetails"];
    ...
    //      //Session["RProviderName"] + " " + Session["RMobileNumber"] + " " + Session["RRechargeAmount"];
    ...
    //                Response.Redirect("~/Masters/Failure.aspx", false);
    ...
    protected void imgbtnGuestcancel_Click(object sender, ImageClickEventArgs e)
    {
        try
        {
            ...
            Response.Redirect("~/Agent/Recharge/Recharge.aspx", false);
        }
                ...
                if (_objDataSet.Tables.Count > 0)
                {
                    if (_objDataSet.Tables[0].Rows.Count > 0)
                    {
                        ...
                        ddlpopDTH.Items.Insert(0, "Please Select");
                         ...
                         if (ddlD2HProvider.SelectedValue != "Please Select")
                        {
                            
                            DataTable dt = _objDataSet.Tables[0];
                            DataRow[] dr = dt.Select("OperatorsName = '" + ddlD2HProvider.SelectedItem.Text + "'");
    ...
    protected void lnkdatacardpop_Click(object sender, EventArgs e)
    {
        mpedatacardpop.Show();
        fnLoadPageData();
        btnPOPData_Click(sender, e);
    }

    private void fnLoadPageData()
    {
        try
        {
            _objMasters = new clsMasters();
            ...
            if (_objDataSet != null)
            {
                if (_objDataSet.Tables.Count > 0)
                {
                    if (_objDataSet.Tables[0].Rows.Count > 0)
                    {
                        ddlpopdatacard.DataSource = _objDataSet.Tables[0];
                        ddlpopdatacard.DataValueField = "OperatorsID";
                        ddlpopdatacard.DataTextField = "OperatorsName";
                        ddlpopdatacard.DataBind();
                        ddlpopdatacard.Items.Insert(0, "Please Select");

                        if (ddlNetConnect.SelectedValue != "Please Select")
                        {

                            DataTable dt = _objDataSet.Tables[0];
                            DataRow[] dr = dt.Select("OperatorsName = '" + ddlNetConnect.SelectedItem.Text + "'");
                            ...
                            ddlpopdatacard.SelectedValue = dt1.Rows[0]["OperatorsID"].ToString();

                        }
                    }
                    else
                    {
                        lblpopdata.Text = "No data found";
                    }
                }
                else
                {
    ...
    protected void getpopularrechargesData()
    {
        try
        {
            _objMasters = new clsMasters();
            _objMasters.ScreenInd = Masters.GetTarrif;
            _objMasters.OperatorsID = Convert.ToInt32(ddlpopdatacard.SelectedValue);
            _objDataSet = (DataSet)_objMasters.fnGetData();
            if (_objDataSet != null)
            {
                if (_objDataSet.Tables.Count > 0)
                {
                    ...
                    {
                        lblpopdata.Text = "";
                        gvdatapop.DataSource = _objDataSet.Tables[0];
                        gvdatapop.DataBind();
                        gvdatapop.Visible = true;

                    }
                    else
                    {
                        lblpopdata.Text = "No data found";
                        lblpopdata.Visible = true;
                        lblmsg.ForeColor = Color.Red;
                        gvdatapop.Visible = false;
                    }
                }
                else
                {
    ...
    protected void btnPOPData_Click(object sender, EventArgs e)
    {
        try
        {
            mpedatacardpop.Show();
            _objMasters = new clsMasters();
            if (ddlpopdatacard.SelectedValue == "Please Select")
            {

            }
            else
            {
                getpopularrechargesData();
            }
