...
public partial class ShowTrips : System.Web.UI.Page
{
                    ...
                    if (dtMain.Rows.Count > 0)
                    {
                        ...
                        foreach (Int16 index in zIndexCount)
                        {
                            if (zIndexCount.Count > 1)
                            {
                                ...
                                sbSeatLayout.Append("<div><table> <tr><td style=\"border-bottom: solid 0px #dadada;\"> " + dtMain.TableName.ToString() + " </td></tr>");
    ...
    private static String[] FilterBuses(bool ac, bool nonAc, bool sleeper, bool semiSleeper, String traveloperator, String sortfield)
    {
        ...
        if (traveloperator != "No Operators")
        {
            strFilterBusesWithDropDowns = "(Travels LIKE '%" + traveloperator + "%')";
        }
        HttpContext.Current.Session["travelOperatorSelected"] = traveloperator;
