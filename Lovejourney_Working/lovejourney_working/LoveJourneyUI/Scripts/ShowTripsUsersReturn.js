function LoadDestinations() {
    ...
    $.ajax(
                {
                    type: "POST",
                    url: "../../Users/Bus/Show_Trips_ReturnJourney.aspx/GetDestinations",
                    data: "{sourceId: '" + ddl + "'}",
...
function LoadRoutes(evt, arg) {
    ...
    $.ajax(
                {
                    type: "POST",
                    url: "../../Users/Bus/Show_Trips_ReturnJourney.aspx/GetRoutes",
                    ...
                    success: function (msg) {
                        ...
                        $("#ddlOperator").multiselect(
                                                {
                                                    noneSelectedText: "All Operators",
                                                    selectedList: 1,
...
function LoadSeatLayout(evt, arg) {
    ...
    $.ajax(
                {
                    type: "POST",
                    url: "../../Users/Bus/Show_Trips_ReturnJourney.aspx/GetSeatLayout",
                    data: "{args: '" + arg + "'}",
                    contentType: "application/json; charset=utf-8",
                    dataType: "json",
                    async: true,
                    ...
                    success: function (msg) {
                        ...
                        document.getElementById("hdnJourneyDetailsJQ").value = msg.d[4];
...
//            html += "            <div class=\"t_avail\" style=\"valign:bottom;\">";
...
//            html += "            </div>";
            ...
            hdnfields += "<input id=\"hdnSourceIdJQ" + routeid + "\" type=\"hidden\" value=\"" + attribArray[18] + "\" />";
            hdnfields += "<input id=\"hdnSourceNameJQ" + routeid + "\" type=\"hidden\" value=\"" + attribArray[19] + "\" />";
            hdnfields += "<input id=\"hdnDestinationIdJQ" + routeid + "\" type=\"hidden\" value=\"" + attribArray[20] + "\" />";
            hdnfields += "<input id=\"hdnDestinationNameJQ" + routeid + "\" type=\"hidden\" value=\"" + attribArray[21] + "\" />";
            hdnfields += "<input id=\"hdnJourneyDateJQ" + routeid + "\" type=\"hidden\" value=\"" + attribArray[22] + "\" />";

            //Append button

            //            if (attribArray[17] != "0") {
            hdnfields += "<button id=\"btnSelectSeats\" class='t_srch' onclick=\"return LoadSeatLayout(event, $('#hdnAPI" + routeid + "').val() + '~' +$('#hdnlblS" + routeid + "').val() + '~' + $('#hdnTravelsJQ" + routeid + "').val()+ '~' + $('#hdnBusTypeJQ" + routeid + "').val()+ '~' + $('#hdnlblBJQ" + routeid + "').val()+ '~' + $('#hdnServiceNumberJQ" + routeid + "').val()+ '~' + $('#hdnBoardingPointsJQ" + routeid + "').val() + '~' + $('#hdnJourneyTypeJQ" + routeid + "').val() + '~' + $('#hdnSourceIdJQ" + routeid + "').val() + '~' + $('#hdnSourceNameJQ" + routeid + "').val()+ '~' + $('#hdnDestinationIdJQ" + routeid + "').val()+ '~' + $('#hdnDestinationNameJQ" + routeid + "').val()  + '~' + $('#hdnJourneyDateJQ" + routeid + "').val());\">" + "View Seats" + "</button>";
...
function fnGetCancellation(arg) {
    ...
    $.ajax({
        type: "POST",
        url: "../../Users/Bus/Show_Trips_ReturnJourney.aspx/GetCancellationPolicy",
...
function fnGetPoints(arg, element) {
    ...
    var timer = setTimeout(function () {
        ...
        $.ajax({
            type: "POST",
            url: "../../Users/Bus/Show_Trips_ReturnJourney.aspx/GetPoints",
