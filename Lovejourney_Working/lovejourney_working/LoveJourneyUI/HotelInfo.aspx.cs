...
public partial class HotelInfo : System.Web.UI.Page
{
    ...
    protected void Page_PreInit(object sender, EventArgs e)
    {
        if (Session["UserID"] != null)
        {
            ...
            {
                this.MasterPageFile = "~/Users/Hotel/MasterPage.master";
            }
            else if (Session["Role"].ToString() == "Agent")
            {
                this.MasterPageFile = "~/Agent/Hotel/MasterPage.master";
            }
        }
    }
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        lblMsg.Text = "";
        this.Page.Title = "LoveJourney - HotelInfo";
        ...
        if (!IsPostBack)
        {
            if (Session["SelectedHotelParams"] != null && Session["SearchParams"] != null)
            {
                ...
                checkInDate = lblCheckIn.Text.ToString();
                checkOutDate = lblCheckOut.Text.ToString();

                string s = lblCheckIn.Text.ToString();
                string[] result = s.Split('-');
                checkInDate = result[0] + "/" + result[1] + "/" + result[2];



                string s1 = lblCheckOut.Text.ToString();
                string[] result1 = s1.Split('-');
                checkOutDate = result1[0] + "/" + result1[1] + "/" + result1[2];
    ...
    protected void btnSubmit_Click(object sender, EventArgs e)
    {
        try
        {
            //lblMsg.Text = "Room booking failed. Please contact administrator.";
            //return;
            ...
            if (Session["SelectedHotelParams"] != null && Session["SearchParams"] != null)
            {
                ...
                string fromDate =strValues[1].ToString();

                fromDate = fromDate.Replace('-', '/');


                string toDate = strValues[2].ToString();
                ...
                if (dsHotelProvisional.Tables.Contains("HotelProvisional"))
                {
                    ...
                    if (dt != null && dt.Rows.Count != 0)
                    {
                        ...
                        if (errorResult == "")
                        {
                            DateTime dt1 = DateTime.Parse(strValues[1], System.Globalization.CultureInfo.GetCultureInfo("en-gb"));
                            DateTime dt2 = DateTime.Parse(strValues[2], System.Globalization.CultureInfo.GetCultureInfo("en-gb")); 
                            string st = ""; string hotelRefNo = Common.GenerateHotelRefNo();

                            //GuestUser
                            if (Session["UserID"] == null)
                            {
                                st = InsertProvisional(
                                    hotelRefNo, cityName, dt1, dt2
                                    ...
                                    , custCity, custZipCode, custState, custCountry, "0", null, null
                                    , hotelName, hotelAddress, hotelStar, hotelTotalFare.Split('~')[0].ToString()
                                    , hotelTotalFare.Split('~')[1].ToString()
                                    );

                                if (st == "Success")
                                {
                                    Session["HotelRefNo"] = hotelRefNo;
                                    Response.Redirect("~/Pay.aspx?service=Hotel", false);
                            ...
                            else
                            {
                                ...
                                {
                                    st = InsertProvisional(hotelRefNo, cityName, dt1, dt2
                               , noOfRooms, noOfAdults, noOfChilds, Session["SearchParams"].ToString(), hotelId, webService, ratePlanCode,
                               roomTypeCode, allocavailResult, allocidResult
                               , roomType, wsKey, roomBasis, title, firstName, middleName, lastName, phoneNumber, emailId, custAddressLine
                               ...
                               , markUp);
                                    ...
                                    {
                                        Session["HotelRefNo"] = hotelRefNo;
                                        Response.Redirect("~/Agent/Hotel/HotelTicket.aspx", false);
                                    }
                                    else { lblMsg.Text = "Please Try Again."; }
                                }
                                //For Admin or CSE or User
                                  ...
                                  , hotelTotalFare.Split('~')[1].ToString());
                                    ...
                                    if (Session["UserID"] != null)
                                    {
                                        if (st == "Success")
                                        {
                                            ...
                                            {
                                                Session["HotelRefNo"] = hotelRefNo;
                                                Response.Redirect("~/Users/Hotel/HotelTicket.aspx", false);
                                            }
                                        }
                                        else { lblMsg.Text = "Please Try Again."; }
                                    }
        ...
        , int? createdBy, int? modifiedBy, string hotelName, string hotelAddress, string hotelStar, string hotelTotalFare, string hotelTotalFareDetails)
    {
        try
        {
            ...
            Session["Amount"] = hotelTotalFare;

            bool b = obj.AddHotelProvisional();
